...
class DFCDataset(Dataset):
    ...
    def __init__(
        ...
        if self.balanced_classes:
            ...
            if self.add_cacao:
                self.observations = pd.read_csv(
                    # os.path.join(base_dir, mode + "_observations_cacao.csv"),
                    os.path.join(base_dir, mode + "_observations_rubber.csv"),
                    header=None,
    ...
    def __getitem__(self, idx, s2_bands=S2Bands.ALL, transform=True, normalize=True):
        ...
        if self.image_px_size != 256:
            ...
            x_offset, y_offset = np.random.randint(0, 256 - self.image_px_size - 5, 2)
            # x_offset, y_offset = np.random.randint(0, 256 - self.image_px_size - 5, 2)
        ...
        if self.mode != "sen12ms":
            ...
            # print(season, obs.Scene, int(obs.ID))

            s2, dfc  = [
                x.astype(np.float32) if type(x) == np.ndarray else x
                for x in self.data.get_s2_dfc(
                    season, obs.Scene, int(obs.ID), s2_bands=s2_bands, dfc_bands=LCBands.DFC, window=window
                )
            ]
            ...
            #     dfc_multilabel.flatten(), num_classes=9
            # ).float()
